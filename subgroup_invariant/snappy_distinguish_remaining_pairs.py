...
def compute_covers_in_parallel(knots, csv_invariants_list, deg, num_workers, debug_level=0):
    """
        ...
        with ProcessPool(max_workers=num_workers) as pool:
            for k in knots:
                if (k not in already_computed_invariants.keys() 
                    ...
                    future = pool.schedule(compute_covers, args=(k, csv_invariants_list, deg, debug_level, lock))
...
if __name__ == "__main__":
    ...
    knots = get_knots_to_compute(previous_groups_csv=previously_computed_groups_csv, columns=["group", "invariant"])
